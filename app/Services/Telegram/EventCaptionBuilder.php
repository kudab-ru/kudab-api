...
final class EventCaptionBuilder
{
     ...
     * @param  int|null  $itemId  запись очереди: её номер уходит в метку
     *                            `utm_content` — зачем, см. [[PostLink]]
     */
    ...
    private static function kindEmoji(array $raw): string
    {
        ...
        if ($byInterest !== '') {
            return self::titleNamesTheme($title, $byInterest) ? '' : $byInterest;
        }
     ...
     * и к самим тегам <blockquote>, и разметка уехала бы в текст поста. Режем
     * по границе фразы — цитата, оборванная посреди слова, читается совсем
     * плохо: полоска слева обещает дословность.
     */
     ...
     *
     * Зачем метка и что в ней — см. [[PostLink]]. Превью в редакторе шаблонов
     * записи не имеет, и метки в нём нет: мерить там нечего, а лишний хвост в
     * ссылке сбивал бы с толку.
     */
