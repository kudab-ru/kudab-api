...
final class EventCaptionBuilder
{
     ...
     * видно с первого взгляда, а 🎵 и 🎶 рядом уже сливаются — поэтому у
     * одного события они выпасть не могут, `pop` и `music` взаимно исключены
     * правилом надтемы.
     */
        ...
        foreach ($ordered as $slug) {
            ...
            if ($slug === 'music') {
                foreach ($ordered as $other) {
                    if (isset(self::MUSIC_GENRES[$other])) {
                        return self::INTEREST_EMOJI[$other];
                    }
                }
            }
