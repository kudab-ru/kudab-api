 ...
 * и без текста.
 *
 * НО НЕ ДО ПОСЛЕДНЕЙ СЕКУНДЫ. Раньше здесь было написано «наполняет их
 * композитор перед самой отправкой», и это оказалось слишком буквально: между
 * фиксацией состава и выходом поста проходило шесть секунд (замер по записям
 * 214 и 215). Теперь состав собирает `broadcast:prepare-digests` за сутки до
 * слота — за это время пул почти не меняется, зато у человека появляется
 * вечер, чтобы посмотреть, что рубрика выбрала. Подпись по-прежнему
 * пересобирается перед отправкой: диапазон дат, цены и время должны быть
 * свежими.
 *
