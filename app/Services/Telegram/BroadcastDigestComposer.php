...
final class BroadcastDigestComposer
{
    ...
    private function buildCaption(
        ...
        // ДВЕ ФОРМЫ ШАПКИ, и выбирает между ними наличие счёта.
        //
        // Со счётом — одним предложением: «На выходных в Воронеже 136 событий.»
        // Срок из неё уходит: «на выходных» и «в эти дни» уже сказали когда, а
        // «26–27 сентября» рядом с ними это та же мысль во второй раз. Точка в
        // конце нужна, потому что следом встык идёт подводка.
        //
        // Без счёта — как было: «Спектакли недели · 25 сентября – 2 октября».
        // У тематических рубрик подвал ведёт на страницу категории, где нет
        // фильтра по датам, поэтому счёта у них нет и не будет: решение
        // владельца — категорийные страницы работают на поиск.
        // Город в шапке ТОЛЬКО в форме со счётом. Прежнее решение «город не
        // пишем — канал городской» верно для шапки-заголовка: там он занимал
        // место, ничего не сообщая. В предложении «На выходных в Воронеже 136
        // событий» он держит фразу: без него выходит «На выходных 136
        // событий» — верно, но звучит как строка отчёта.
        $city = CityCase::prepositional($broadcast->chat?->city?->slug);

        if ($total !== null && $total > 0) {
            // ГОРОД — по решению рубрики, не всегда. Прежнее «город не пишем,
            // канал городской» верно: каждый пост канала и так про Воронеж.
            // Но «На выходных в Воронеже — 134 события» держится городом, а
            // «Бесплатно в эти дни в Воронеже» спотыкается о два «в» подряд.
            // Поэтому ключ на рубрике, а не правило на всех.
            $where = ! empty($theme['city_in_head']) && $city !== null ? ' в '.$city : '';

            // Тире, а не пробел: у половины рубрик заголовок кончается
            // временем («в эти дни»), и без разделителя число к нему
            // прилипает — «Бесплатно в эти дни 26 событий».
            $head = trim(($theme['emoji'] ?? '').' <b>'
                .$this->escape($headline.$where.' — ')
                .$this->escape($this->plural($total, $forms[0] ?? 'событие', $forms[1] ?? null, $forms[2] ?? null))
                .'.</b>');
        } else {
