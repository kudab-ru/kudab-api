...
class TelegramChatBroadcastService
{
    ...
    /**
     * Сколько дней не предлагать снова отклонённое (skipped сюда не входит —
     * см. pickBestEventIdForChat).
     *
    ...
    public function collectDueSingleRuns(
        ...
        foreach ($broadcasts as $broadcast) {
            ...
            // Просрочка отнимает у поста ДЕНЬ, а не сам пост: отправлять его
            // уже стыдно — подписчик увидит анонс вчерашнего дня.
            //
            ...
            if ($dueAt && $dueAt->lt($now->copy()->subHours(self::OVERDUE_CUTOFF_HOURS))) {
                ...
                if ($item->kind === TelegramChatBroadcastItem::KIND_VENUE) {
                    ...
                    // СЛОТА МОЖЕТ НЕ БЫТЬ — и с настройкой `fill_lead_days`
                    // это не редкость: утренние слоты горизонта заняты лентой,
                    // а поздние дальше границы упреждения закрыты намеренно.
                    //
                    // Оставить портрет просто без дня нельзя. Запись без
                    // момента доставка пропускает по ОБЩЕМУ суточному окну
                    // ($hasOwnMoment ниже), то есть портрет уехал бы в
                    // произвольный час первым же подходящим тиком и съел окно
                    // у дневного событийного поста — ровно то поведение, ради
                    // отмены которого портрету и дали слот.
                    //
                    // Поэтому придерживаем на час: за это время наполнитель
                    // ленты (он ходит ежечасно и разбирает очередь ожидания)
                    // назначит день штатно.
                    if ($slot === null) {
            ...
            // (isSingleRunDue); уже-в-полёте ревью-статусы — мимо него.
            ...
            if ($hasReadyCaption) {
                ...
                // моменту заморожен (его ставит prepare-digests заранее), а
    ...
    private function digestWantsText(TelegramChatBroadcastItem $item, TelegramChatBroadcast $broadcast): bool
    {
        return $broadcast->ai_text
            ...
            //
            // ПОЧЕМУ НЕ ТРЕБУЕМ ФРАЗУ КАЖДОМУ СОБЫТИЮ. Гейт в парсере строку
            // не чинит, а ВЫБРАСЫВАЕТ: у события состав совпал, а своей фразы
            // нет. Потребуй мы полного покрытия — заказывали бы текст снова и
            // снова ради строки, которую гейт снимет опять, и платили бы за
            // каждый круг. Случай виден в админке меткой «без фразы» у строки
            // состава, а дозаказать его можно кнопкой «Написать заново» — она
            // переспрашивает и при готовом тексте. Решение человека, не цикла.
    ...
    public function applyDigestDraft(TelegramChatBroadcastItem $item, array $draft): void
    {
        ...
        if ($theme !== '') {
            ...
            // Сколько знаков осталось на фразу под ЭТОТ состав. Пишет api, читает
            // парсер: только здесь известна вся арифметика подписи — шапка со
            // счётом, подвал, настоящие названия и строки фактов. Порог в самом
            // парсере один на все рубрики, а бюджет у трёх строк и у пяти
            // отличается вдвое.
    ...
    public function fillFeedDays(TelegramChatBroadcast $broadcast, Carbon $now): array
    {
        ...
        // выбирает накануне слота (prepare-digests), а на неделю вперёд
        // стоит пустой бронью — её слот для правила соседства выглядит
        // как «тема неизвестна» (null). Почему так поздно — см.
        // [[BroadcastDigestBooking]]. Цена — сосед подборки не знает,
        // что она назовёт; принято сознательно.
        //
    ...
    /** Ключ слота — один на все сравнения «занято ли место в ленте», см. [[BroadcastSlotPlanner]]. */
