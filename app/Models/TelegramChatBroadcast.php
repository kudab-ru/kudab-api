...
class TelegramChatBroadcast extends Model
{
    ...
    /**
     * За сколько дней вперёд заполняется ПОЗДНИЙ слот дня.
     *
     * ЗАЧЕМ. Половина афиши объявляется поздно: по замеру ВК медиана форы
     * анонса — трое суток, 62% событий появляются ближе трёх дней до начала.
     * Лента же собирается на две недели вперёд, поэтому дальний слот выбирает
     * из малой доли того, что там в итоге будет, и хорошее событие, объявленное
     * в среду, в собранную неделю уже не попадёт: свободных слотов нет.
     *
     * Решение — не «какие события ставить во второй слот» (пул у слотов один и
     * тот же, делить его нечем), а КОГДА этот слот заполняется. Первый слот дня
     * остаётся плановым: он собирается на весь горизонт, и человек видит неделю
     * заранее. Поздний решается за сутки-двое, и выбирает уже из того, что
     * объявлено.
     *
     * NULL — настройка не задана: поздний слот заполняется вместе с первым, как
     * было до появления ручки.
     */
    ...
    /**
     * Форма поста на конкретный ДЕНЬ: весь день одной формой, назавтра другая.
     *
     * Сначала выбиралось по номеру поста, но номера идут с пропусками —
     * удалённые и пропущенные записи. Замер на живой очереди: 248, 249, 250,
     * 252, 253, 254, 255, 257 — по чётности это A B A A B A B B, то есть пары
     * одинаковых форм подряд. Чересполосица внутри дня к тому же читается как
     * случайность, а день одной формой — как решение редактора.
     *
     * Считаем по суткам от эпохи, а не по дню месяца: на стыке месяцев и лет
     * не будет повтора. Дата одна и та же при любой пересборке, поэтому пост
     * не «прыгает» между формами.
     */
