...
class Event extends Model
{
     ...
     * Пока в таблице связи нет колонок, совпадающих по имени с колонками
     * записи, неоднозначности не возникает — проверено запросом, `updated_at`
     * разрешается однозначно. Это и есть причина, по которой в схеме связи
     * намеренно нет `updated_at`, `broadcast_id`, `status` и `posted_at`:
     * добавить любую из них — значит сломать все запросы анти-дублей разом,
     * причём падением, а не деградацией. Колонки записи всё равно стоит писать
     * полным именем: это дешевле, чем помнить правило.
     */
