...
class Event extends Model
{
    ...
    /**
     * Записи очереди, чьё ВЕДУЩЕЕ событие — это.
     *
     * Отвечает на вопрос «что отправлять»: подпись, картинки, задача боту. На
     * вопрос «показывал ли канал это событие» отвечает broadcastPosts(): у
     * подборки недели ведущего события нет вовсе, а рассказывает она о пяти.
     */
    ...
    /**
     * Все посты канала, которые рассказывали об этом событии, — включая те, где
     * оно одно из нескольких.
     *
     * Это и есть память канала о событии: на ней стоят все слои анти-дублей.
     * Читать через неё, а не через broadcastItems(), иначе событие, названное
     * подборкой, останется кандидатом обычной ленты и выйдет вторым постом.
     *
     * ВАЖНО при использовании внутри whereDoesntHave: подзапрос джойнит ДВЕ
     * таблицы, а Laravel выводит имена колонок как есть, без квалификации.
     * Совпадающая колонка в них уже есть — `created_at`, и она стреляет:
     * `orderByRaw('COALESCE(posted_at, publish_at, created_at)')` в гарде
     * дверей постановки уронил запрос на «column reference "created_at" is
     * ambiguous», это поймал тест. Поэтому правило простое: ЛЮБУЮ колонку
     * внутри такого запроса писать полным именем или через алиас.
     *
     * По той же причине в схеме связи намеренно нет `updated_at`,
     * `broadcast_id`, `status` и `posted_at` — иначе на них упали бы уже
     * существующие читатели анти-дублей, и упали бы разом.
     */
    public function broadcastPosts(): BelongsToMany
    {
        return $this->belongsToMany(
            TelegramChatBroadcastItem::class,
            'telegram.chat_broadcast_item_events',
            'event_id',
            'item_id',
        )->withPivot('position');
    }
