...
// Сборка подборки заранее — за сутки до слота, а не в минуту отправки. До
// этого состав и текст появлялись внутри доставки, и между «состав
// зафиксирован» и «пост в канале» проходили секунды: 6 у записи 214, 28 у 215.
// Посмотреть на состав, а тем более поменять в нём позицию, было физически
// некогда. Замораживается только состав — подпись доставка пересобирает, как
// и раньше. Часовой тик: граница «за N часов до слота» приходится на
// произвольную минуту.
